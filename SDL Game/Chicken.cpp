...
void Chicken::set_clips() { // tạo ảnh động của kẻ địch 
    for (int i = 0; i < 18; i++) {
...
void Chicken::Show(SDL_Renderer* screen) { // load kẻ địch lên screen 
    if (come_back_ == true) {
        frame_++;
        if (frame_ / 5 >= 18) {
            frame_ = 0;
...
void Chicken::HandleBullet(SDL_Renderer* screen) { // load đạn của kẻ địch lên screen 
...
void Chicken::InitBullet(bullet* p_bullet, SDL_Renderer* screen) { // đạn của kẻ địch 
    if (p_bullet != NULL) {
...
void Chicken::Move() { // di chuyển của kẻ địch 
    if (come_back_ == true) {
        rect_.y += y_val_;
        ...
        if (rect_.x <= -WIDTH_CHICKEN ) {
            status_.turn_right = 1;
            status_.turn_left = 0;
        }
        else if (rect_.x >= SCREEN_WIDTH + WIDTH_CHICKEN) {
            status_.turn_left = 1;
            status_.turn_right = 0;
        }
    }
}
void Chicken::RemoveBullet(const int& idx) { // xóa đạn của kẻ địch 
    for (int i = 0; i < bullet_list_.size(); i++) {
