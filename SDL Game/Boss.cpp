...
void Boss::set_clips() { // tạo ảnh động cho boss 
    for (int i = 0; i < 10; i++) {
...
void Boss::Show(SDL_Renderer* screen) { // load boss lên screen
    frame_++;
    if (frame_ / 5 >= 10) {
        frame_ = 0;
    }
...
void Boss::MakeBullet(SDL_Renderer* screen) { 
    for (int i = 0; i < bullet_list_.size(); i++) { // load đạn của boss lên screen 
...
void Boss::InitBullet(bullet* p_bullet, SDL_Renderer* screen) { // đạn của boss 
    if (p_bullet != NULL) {
...
void Boss::show_heart_boss(SDL_Renderer* screen, int x, int y, int w, int h) { // load ảnh thanh máu của kẻ địch 
