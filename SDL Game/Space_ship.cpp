...
void space_ship::Control(SDL_Event event, SDL_Renderer* screen, Mix_Chunk* bullet_sound[4], int bullet_level, Mix_Chunk* g_sound_level_up) {
    if (event.type == SDL_KEYDOWN && event.key.repeat == 0) { // Sự kiện khi ấn phím di chuyển 
    ...
    else if (event.type == SDL_KEYUP && event.key.repeat == 0) { // Sự kiện khi nhả phím di chuyển 
        switch (event.key.keysym.sym) {
        case SDLK_w:
            y_val_ += MAIN_SPEED;
            break;
    ...
    if (event.type == SDL_KEYDOWN && event.key.repeat == 0) {
        if (event.key.keysym.sym == SDLK_SPACE) { // Sự kiện khi ấn phím space (Bắn đạn)
            if (status_ == true) {
        ...
        if (event.key.keysym.sym == SDLK_RETURN) { // Sự kiện khi ấn phím enter (Hồi sinh)
            if (heart_ > 0) {
                if (status_ == false) {
...
void space_ship::Show(SDL_Renderer* screen) { // load ảnh nhân vật chính ra màn hình 
    if (status_ == true) {
        Render(screen);
    }
}
void space_ship::Move() { // Di chuyển nhân vật chính 
    if (status_ == true) {
        rect_.x += x_val_;
...
void space_ship::HandleBullet(SDL_Renderer* screen) { // Load bắn đạn của nhân vật chính
...
void space_ship::RemoveBullet(const int& idx) { // Xoá đạn
    for (int i = 0; i < bullet_list_.size(); i++) {
